...
(p/let-rules [r1 "a(x), b(x) -> b(x) @ 1"]
  (let [[lhs-rhs created-agents removed-agents] (lang/pair-exprs (:lhs r1) (:rhs r1))
    ...
    #_(is (= lhs-rhs [[(-> r1 :lhs second) (-> r1 :rhs first)]]))
    (is (= created-agents '()))
    #_(is (= removed-agents [(-> r1 :lhs first)]))
    (is (= (count eas) 1))
    (p/let-exprs [e1 "a(x), b(x)", e2 "b(x)"]
      (let [[mm lf] (maps/matching-and-lift-map [r1] e1), lhs (:lhs r1)]
        ...
        #_(is (= mm {r1 {#{a-r1} [{a-r1 a-e1}], #{b-r1} [{b-r1 b-e1}]}}))
        #_(is (= lf {a-e1 {"x" [{:rule r1, :complex #{a-r1}, :codomain [[a-e1 "x"]]}]},
                     b-e1 {"x" [{:rule r1, :complex #{b-r1}, :codomain [[b-e1 "x"]]}]}}))
        (let [chamber1 (chamber/make-chamber [r1] e1 1 [])
              ...
              chamber3 (-> (chamber/gen-event chamber1)
                           (update-in [:mixture]
                                      #(vary-meta % conj
                                                  {:complexes (lang/compute-complexes %)})))]
          (is (= mm chamber1-mm))
...
(p/let-rules [r1 "a(x) -> b(x) @ 1"]
  (p/let-exprs [e1 "1000 * a(x)"]
    (let [initial-chamber (chamber/make-chamber [r1] e1 1 [])
